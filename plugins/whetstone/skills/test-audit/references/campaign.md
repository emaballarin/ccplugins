...
The examples come from the campaign this procedure was distilled from, which
pruned the test surface of a messaging-platform integration.

## 1. Baseline

Record the subsystem's test and support line counts and every test file's
pass/fail state at a pinned commit. Keep baseline failures in their own list: in
the source campaign, all three were real delivery bugs, not stale tests.
...
prefixes — in the source campaign: accounts, commands, context, dispatch,
...
Judge a test by its assertions, not its name. One test in the source campaign,
named for retiring a progress window, asserted the window was _not_ cleared.
...
pass, starting from the ledger, looks for the redundant **layer**. In the source
...
the **keeper** suite for each contract. Prefer the real transport boundary with a
...
and the operator has approved the plans. Nothing is edited before that approval.

## 5. Cutover

Edit lane by lane. Serialise changes to shared harnesses and support files
...
Register moved suites wherever the project enumerates tests — CI routing, test
inventories, size baselines. Put durable test-ownership rules in the
...
source campaign's review found nine real gaps and one unreachable assertion.
...
**control** run that reverts the fix and shows the old behaviour. Record
...
A campaign on a branch outlives many commits on the default branch. Merge the
default branch rather than rebasing a long, many-commit campaign. When the
default branch modified a file the campaign deleted, keep the deletion. Port the
new contract into the keeper instead, and confirm every new regression the
default branch added still has a home. Rerun the whole subsystem suite and
repeat live proof on the merged head.

Expect review tooling to see a truncated file list on a diff this large. Record
the operator's decisions on compatibility flags in the change's evidence rather
than editing gates.

Hand off with the skill's report, plus:
