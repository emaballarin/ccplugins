...
After installation, nine skills are available under the `ccsci` namespace and two subagents
...
| `paper-review` (offline PDF export only)               | `pandoc` (or `pip install markdown`) plus `wkhtmltopdf` (or `pip install weasyprint`) |
...
| **paper-review**          | reviewing a paper for a venue, PDF and review form in hand       | User-invoked (`/ccsci:paper-review`): Claude is the **second reader** of a review the reviewer writes. Frame venue + rubric → read the whole paper → test the reviewer's impression against the text → arc, claim ledger, line-anchored findings (line ref **and** quoted phrase), missing list, decisive experiment → argue both rubric boundaries → constrain and error-check the summary, title and review, in that order. Literature enters only after its primary source is checked; `unverified` items are kept in view until resolved. Review prose from Claude only on "draft it". In-chat by default; context dump and A4 PDF notes on request. | `pandoc` or `markdown`, plus `wkhtmltopdf` or `weasyprint` (PDF export only)                                                           |
...
- **Adjacent, not duplicate** — `figure-style` complements the upstream `dataviz` skill
  ...
  build toolchain.
- **Original** — `paper-review` derives from no upstream skill; it captures a human-driven review workflow and ships user-invoked (interaction-mode, whole-session blast radius).
