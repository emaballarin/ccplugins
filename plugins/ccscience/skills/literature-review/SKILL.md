...
The public helpers are `verify_dois`, `crossref_lookup`, `search_openalex`, `expand_citations`, `extract_dois`, `style_pass`, `resolve_published` / `resolve_published_all`, `dedupe_records`, `to_bibtex`, and `bibtex_tidy`. Each call is a short `python` run that imports the module and prints JSON you read back. Two optional env vars tune the polite path: `LITREVIEW_CONTACT_EMAIL` (Crossref/doi.org polite pool; a default is used otherwise) and `OPENALEX_API_KEY` (raises OpenAlex's per-request budget; without it, OpenAlex calls go out unauthenticated and still work, subject to shared rate limits).
