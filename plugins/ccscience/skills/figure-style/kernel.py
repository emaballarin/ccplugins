...
def apply_figure_style(*, frame="open", font=None, sizes=(8, 7, 6), grid=False):
    ...
    # Best-effort: register any locally-installed fonts (e.g. conda mscorefonts in
    # $CONDA_PREFIX/fonts) that sit off matplotlib's default scan path. Silently
    # skipped when the directory is absent or a font fails to load.
    import contextlib
    ...
    # matplotlib's RcParams.update stub types keys as a Literal union; a plain
    # str-keyed dict is valid at runtime but trips the checker.
    mpl.rcParams.update(rc)  # pyright: ignore[reportCallIssue, reportArgumentType]
...
def panel_letter(ax, letter, dx=-0.18, dy=1.02, case="lower", fontsize=None):
    ...
    ax.text(dx, dy, s, transform=ax.transAxes, fontweight="bold", fontsize=fontsize, va="bottom", ha="left")
...
def focal_palette(labels, focal, focal_color, other="muted", base_colors=None):
    ...
    if other == "grey":
        ...
        ramp = [
            mcolors.to_hex((v, v, v)) for v in ([0.55] if nf == 1 else [0.80 - 0.35 * i / (nf - 1) for i in range(nf)])
        ]
        rest, k = [], 0
        for l in labels:
...
def bar_with_points(
    ax, x, ymat, labels, colors, jitter=0.08, show_points=True, errorbar=None, point_alpha=0.5, point_size=8
):
    ...
    ax.bar(x, means, color=colors, width=0.7, edgecolor="none", yerr=err, error_kw={"elinewidth": 0.8, "capsize": 0})
    if show_points:
...
def goodness_arrow(ax, text="higher = better", loc="upper left", axis="y", fontsize=None):
    ...
    ax.text(pos[0], pos[1], arrow + text, transform=ax.transAxes, fontsize=fontsize, color=META_GREY, ha=ha, va=va)
...
def panel_crops(fig, dpi=None, pad_px=6, bbox_inches=None, pad_inches=None):
    ...
    top-left, matching PIL's ``Image.crop``). Panels are detected as bold single-character ``Text``
    ...
    *consult rcParams* (so under :func:`apply_figure_style` it resolves to
    ...
    boxes are clamped to the saved image extent regardless.
        ...
        ...     img.crop(box).save(f"panel_{letter}.png")  # then Read each crop
    """
    import matplotlib as mpl
