...
| `test_evidence_grades.py`  | Every `parml` tier-catalogue item carries a grade, every tier-A/B item also declares a quality exposure, both ladders match what `evidence-grades.md` documents, and every `A#`/`B#`/`C#`/`P#` cross-reference resolves. |

## When a check fails

The message names the offending file and value. Two cases need a test edit
...
- A genuinely new evidence grade or exposure level → add it to `ALLOWED_GRADES` /
  ...
  in `plugins/paretoml/references/evidence-grades.md`; the test cross-checks the
